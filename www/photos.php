...
if(isset($_GET["u"])){
    ...
    header("location: http://www.spintalks.in");
    exit();	
}
$photo_form = "";
...
if($u == $log_username && $user_ok == true){
	...
	$photo_form .=   '<h3>Hi '.$u.', add your CV here</h3>';
	$photo_form .=   '<b>Choose Type:</b> ';
	...
	$photo_form .=   '</select>';
	$photo_form .=   ' &nbsp; &nbsp; &nbsp; <b>Choose File:</b> ';
	$photo_form .=   '<input type="file" name="photo" accept="image/*" required>';
	$photo_form .=   '<p><input type="submit" value="Upload File Now"></p>';
	$photo_form .= '</form>';
}
...
if(mysqli_num_rows($query) < 1){
	$gallery_list = "This user has not uploaded CV yet.";
} else {
...
<head>
<meta charset="UTF-8">
<title><?php echo $u; ?> Profile</title>
...
function backToGalleries(){
	...
	_("section_title").innerHTML = "<?php echo $u; ?>&#39;s Profile and CV";
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "deleted_ok"){
				alert("This CV has been deleted successfully. We will now refresh the page for you.");
...
<div id="pageMiddle">
  ...
  <h2 id="section_title"><?php echo $u; ?>&#39;s Job Profile</h2>
  <div id="galleries"><?php echo $gallery_list; ?></div>
  <div id="photos"></div>
  <div id="picbox"></div>
  <p style="clear:left;">This CV belong to <a href="user.php?u=<?php echo $u; ?>"><?php echo $u; ?></a></p>
</div>
